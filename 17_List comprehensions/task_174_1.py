...
# но делать что-либо с ним нельзя. Для нормальной работы аналитической программы берётся новый список, который равен
# тому, что пришло. Затем идёт работа с новым списком: если есть отрицательные цены, то программа их зануляет и в конце
...
value = int(input('Сколько нужно цен? '))
original_prices = [random.randint(-10, 20) for _ in range(value)]
new_prices = original_prices[:]

print('Исходный список цен', original_prices)

for i in range(len(original_prices)):
    if new_prices[i] < 0:
        new_prices[i] = 0

print('Измененный список:', new_prices)
