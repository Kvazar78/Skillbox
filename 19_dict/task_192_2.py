...
summ = sum(incomes.values())
print('Общий доход за год составил {} рублей'.format(summ))

min_income = min(incomes.values())

for fruit in incomes:
    if incomes[fruit] == min_income:
        print('\nСамый маленький доход у {0}. Он составляет {1} рублей'.format(fruit, min_income))
        incomes.pop(fruit)
        break
